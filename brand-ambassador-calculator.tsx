...
const BrandAmbassadorCalculator = () => {
  ...
  const calculateEarnings = () => {
    ...
    const commissionRate = CONFIG.commissionRates[formData.performanceLevel as keyof typeof CONFIG.commissionRates];
    
    // Calculate network size
    const sportMultiplier = CONFIG.networkSizeMultipliers[formData.sport as keyof typeof CONFIG.networkSizeMultipliers];
    const yearsMultiplier = CONFIG.yearsNetworkFactor[formData.yearsPlayed.length as keyof typeof CONFIG.yearsNetworkFactor] || 1;
    const conferenceQuality = CONFIG.conferenceNetworkQuality[formData.conference as keyof typeof CONFIG.conferenceNetworkQuality];
    
    // Get average claim value for their network
    const averageClaimValue = CONFIG.averageClaimValues[formData.sport as keyof typeof CONFIG.averageClaimValues][formData.performanceLevel as keyof typeof CONFIG.commissionRates] * conferenceQuality;
    ...
    Object.keys(scenarios).forEach(scenario => {
      const conversions = scenarios[scenario as keyof typeof scenarios].conversions;
